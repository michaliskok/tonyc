...
(* Intermediate code for arguments in function calls.
   This routine saves the argument result in a temporary.
   Returns both the expr properties_variable and the properties_variable for the argument *)
let icode_fun_arg expr =
  let w = newTemporary expr.typ in
  let save_q = genQuad Q_assign expr.place Q_empty (Q_entry w) in
  let arg_var = { default_properties with code = save_q :: expr.code; place = Q_entry w } in
  (expr, arg_var)
...
let icode_fun_call (res: Types.typ) (ent : Symbol.entry)  (args : properties_variable list) =
  ...
  let rec gen_arg_code acc = function
    | ([], pars) -> pars @ acc
    | (h::t, pars) -> gen_arg_code (h.code @ acc) (t,pars)
  in 
  ...
  let rec gen_par_code acc = function
    ...
    | (fp1::fps, ar1::ars) ->
	 ...
	 | ENTRY_parameter parInfo ->
	    ...
	    (* When arguments are passed, each argument's place is stored in a temporary.
	       If parameter passes by reference then do not pass the temporary,
               pass the previous place, and also remove the assignment quad from the arg *)
	    if pass_mode = PASS_BY_REFERENCE then
	      let par = List.hd ar1.code in
	      let par_place = par.x in 
	      let new_quad = genQuad Q_par par_place (Q_pass pass_mode) Q_empty in
	      ar1.code <- List.tl (ar1.code);
...
let icode_for_stmt (init, cond, incr) body =
  ...
  (* Backpatch body's next with the condition's first quad *) 
  backpatch body.next cond_first.label;
